from __future__ import annotations

import csv
import hashlib
import mimetypes
from dataclasses import asdict, dataclass
from datetime import datetime, timezone
from pathlib import Path
from typing import Dict, Iterable, List, Optional
from urllib.parse import unquote, urljoin, urlparse

from floriano.utils.execution_logger import ExecutionTimer, PipelineExecutionLogger


@dataclass(frozen=True)
class DocumentDownloadRecord:
    run_id: str
    upstream_pdf_links_run_id: str
    competencia: str
    source_pdf_path: str
    source_pdf_sha256: str
    page_number: int
    uri: str
    uri_sha256: str
    link_type: str
    duplicate_uri_count: int
    download_attempted: bool
    download_status: str
    http_status: Optional[int]
    content_type: Optional[str]
    content_disposition: Optional[str]
    file_name_detected: Optional[str]
    saved_path: Optional[str]
    file_size_bytes: int
    file_sha256: Optional[str]
    already_existed: bool
    was_downloaded_now: bool
    is_duplicate_uri: bool
    is_duplicate_file: bool
    conflict_detected: bool
    error_type: Optional[str]
    error_message: Optional[str]
    downloaded_at_utc: str


class DirectDocumentDownloader:
    """
    Downloader RAW de documentos diretos extraídos de pdf_links.

    Entrada:
    - data/raw/pdf_links/pdf_links_latest.csv

    Filtro:
    - link_type == downloadarquivo

    Saída:
    - data/raw/document_downloads/run_id=<RUN_ID>/document_downloads.csv
    - data/raw/document_downloads/run_id=<RUN_ID>/document_downloads.parquet
    - data/raw/document_downloads/document_downloads_latest.csv

    Arquivos:
    - data/landing/downloads/YYYY_MM/comprovantes/<uri_sha256>.<ext>

    Regras:
    - idempotente;
    - overwrite=False por padrão;
    - skip_existing=True por padrão;
    - deduplica por uri_sha256 antes de baixar;
    - deduplica por file_sha256 após download;
    - não grava em manual_upload;
    - registra sucessos, skips, falhas HTTP, falhas de conexão e content-type inesperado.
    """

    MODULE = "floriano.raw.direct_document_downloader"
    CLASS_NAME = "DirectDocumentDownloader"

    REQUIRED_COLUMNS = {
        "run_id",
        "competencia",
        "source_path",
        "source_sha256",
        "page_number",
        "uri",
        "uri_sha256",
        "link_type",
    }

    EXPECTED_CONTENT_PREFIXES = (
        "application/pdf",
        "image/",
        "application/octet-stream",
    )

    def __init__(
        self,
        pdf_links_path: str | Path = "data/raw/pdf_links/pdf_links_latest.csv",
        landing_downloads_dir: str | Path = "data/landing/downloads",
        output_dir: str | Path = "data/raw/document_downloads",
        logger: Optional[PipelineExecutionLogger] = None,
        session: Optional[object] = None,
        timeout_seconds: int = 60,
        base_url: Optional[str] = None,
        skip_existing: bool = True,
        overwrite: bool = False,
        dry_run: bool = False,
        limit: Optional[int] = None,
        require_parquet: bool = False,
    ) -> None:
        if overwrite and skip_existing:
            raise ValueError("Configuração inválida: overwrite=True e skip_existing=True.")

        self.pdf_links_path = Path(pdf_links_path)
        self.landing_downloads_dir = Path(landing_downloads_dir)
        self.output_dir = Path(output_dir)
        self.logger = logger or PipelineExecutionLogger()
        self.timeout_seconds = timeout_seconds
        self.base_url = base_url
        self.skip_existing = skip_existing
        self.overwrite = overwrite
        self.dry_run = dry_run
        self.limit = limit
        self.require_parquet = require_parquet

        if session is None:
            import requests
            self.session = requests.Session()
        else:
            self.session = session

        self.pdf_link_rows: List[Dict[str, str]] = []
        self.records: List[DocumentDownloadRecord] = []

    @staticmethod
    def _utc_now() -> str:
        return datetime.now(timezone.utc).isoformat(timespec="milliseconds")

    @staticmethod
    def sha256_bytes(content: bytes) -> str:
        return hashlib.sha256(content).hexdigest()

    @staticmethod
    def sha256_text(value: str) -> str:
        return hashlib.sha256(value.encode("utf-8")).hexdigest()

    @staticmethod
    def is_expected_content_type(content_type: Optional[str]) -> bool:
        if not content_type:
            return False

        normalized = content_type.lower().split(";")[0].strip()
        return any(normalized.startswith(prefix) for prefix in DirectDocumentDownloader.EXPECTED_CONTENT_PREFIXES)

    @staticmethod
    def extension_from_content_type(content_type: Optional[str]) -> str:
        if not content_type:
            return ".bin"

        clean = content_type.lower().split(";")[0].strip()

        mapping = {
            "application/pdf": ".pdf",
            "image/jpeg": ".jpg",
            "image/jpg": ".jpg",
            "image/png": ".png",
            "image/webp": ".webp",
            "image/gif": ".gif",
            "text/html": ".html",
            "application/octet-stream": ".bin",
        }

        if clean in mapping:
            return mapping[clean]

        guessed = mimetypes.guess_extension(clean)
        return guessed or ".bin"

    @staticmethod
    def filename_from_content_disposition(content_disposition: Optional[str]) -> Optional[str]:
        if not content_disposition:
            return None

        parts = [p.strip() for p in content_disposition.split(";")]

        for part in parts:
            if part.lower().startswith("filename*="):
                value = part.split("=", 1)[1].strip().strip('"')
                if "''" in value:
                    value = value.split("''", 1)[1]
                return unquote(value)

            if part.lower().startswith("filename="):
                return part.split("=", 1)[1].strip().strip('"')

        return None

    @staticmethod
    def filename_from_uri(uri: str) -> Optional[str]:
        parsed = urlparse(uri)
        name = Path(parsed.path).name
        return unquote(name) if name else None

    def normalize_uri(self, uri: str) -> str:
        uri = (uri or "").strip()

        if uri.startswith("http://") or uri.startswith("https://"):
            return uri

        if uri.startswith("/") and self.base_url:
            return urljoin(self.base_url.rstrip("/") + "/", uri.lstrip("/"))

        return uri

    @staticmethod
    def is_supported_download_uri(uri: str) -> bool:
        return uri.startswith("http://") or uri.startswith("https://")

    def load_pdf_links(self) -> List[Dict[str, str]]:
        timer = ExecutionTimer()

        self.logger.started(
            layer="raw",
            module=self.MODULE,
            class_name=self.CLASS_NAME,
            method_name="load_pdf_links",
            operation="load_pdf_links",
            message="Carregando pdf_links latest.",
            source_file=str(self.pdf_links_path),
        )

        if not self.pdf_links_path.exists():
            raise FileNotFoundError(f"pdf_links não encontrado: {self.pdf_links_path}")

        with self.pdf_links_path.open("r", encoding="utf-8") as f:
            reader = csv.DictReader(f)
            rows = list(reader)
            fieldnames = set(reader.fieldnames or [])

        missing = self.REQUIRED_COLUMNS - fieldnames

        if missing:
            raise ValueError(f"pdf_links sem colunas obrigatórias: {sorted(missing)}")

        self.pdf_link_rows = rows

        self.logger.success(
            layer="raw",
            module=self.MODULE,
            class_name=self.CLASS_NAME,
            method_name="load_pdf_links",
            operation="load_pdf_links",
            message="pdf_links carregado.",
            records_out=len(rows),
            duration_ms=timer.elapsed_ms(),
        )

        return rows

    def filter_download_links(self) -> List[Dict[str, str]]:
        if not self.pdf_link_rows:
            self.load_pdf_links()

        download_rows = [
            row for row in self.pdf_link_rows
            if row.get("link_type") == "downloadarquivo"
        ]

        if self.limit is not None:
            download_rows = download_rows[: self.limit]

        return download_rows

    @staticmethod
    def group_by_uri_sha256(rows: Iterable[Dict[str, str]]) -> List[Dict[str, object]]:
        grouped: Dict[str, List[Dict[str, str]]] = {}

        for row in rows:
            key = row.get("uri_sha256") or DirectDocumentDownloader.sha256_text(row.get("uri", ""))
            grouped.setdefault(key, []).append(row)

        groups: List[Dict[str, object]] = []

        for uri_sha, items in grouped.items():
            first = items[0]
            groups.append(
                {
                    "uri_sha256": uri_sha,
                    "first_row": first,
                    "duplicate_uri_count": len(items),
                    "is_duplicate_uri": len(items) > 1,
                }
            )

        return groups

    def existing_file_for_uri(self, competencia: str, uri_sha256: str) -> Optional[Path]:
        folder = self.landing_downloads_dir / competencia / "comprovantes"

        if not folder.exists():
            return None

        matches = sorted(folder.glob(f"{uri_sha256}.*"))

        return matches[0] if matches else None

    def target_path_for_download(
        self,
        *,
        competencia: str,
        uri_sha256: str,
        content_type: Optional[str],
    ) -> Path:
        ext = self.extension_from_content_type(content_type)
        folder = self.landing_downloads_dir / competencia / "comprovantes"
        folder.mkdir(parents=True, exist_ok=True)
        return folder / f"{uri_sha256}{ext}"

    def known_file_hashes(self) -> Dict[str, str]:
        known: Dict[str, str] = {}

        base = self.landing_downloads_dir

        if not base.exists():
            return known

        for path in base.rglob("*"):
            if path.is_file() and path.name != ".gitkeep":
                try:
                    content = path.read_bytes()
                    known[self.sha256_bytes(content)] = str(path)
                except Exception:
                    continue

        return known

    def _record(
        self,
        *,
        row: Dict[str, str],
        uri: str,
        uri_sha256: str,
        duplicate_uri_count: int,
        download_attempted: bool,
        download_status: str,
        http_status: Optional[int],
        content_type: Optional[str],
        content_disposition: Optional[str],
        file_name_detected: Optional[str],
        saved_path: Optional[str],
        file_size_bytes: int,
        file_sha256: Optional[str],
        already_existed: bool,
        was_downloaded_now: bool,
        is_duplicate_uri: bool,
        is_duplicate_file: bool,
        conflict_detected: bool,
        error_type: Optional[str],
        error_message: Optional[str],
    ) -> DocumentDownloadRecord:
        return DocumentDownloadRecord(
            run_id=self.logger.run_id,
            upstream_pdf_links_run_id=row.get("run_id", ""),
            competencia=row.get("competencia", ""),
            source_pdf_path=row.get("source_path", ""),
            source_pdf_sha256=row.get("source_sha256", ""),
            page_number=int(float(row.get("page_number") or 0)),
            uri=uri,
            uri_sha256=uri_sha256,
            link_type=row.get("link_type", ""),
            duplicate_uri_count=duplicate_uri_count,
            download_attempted=download_attempted,
            download_status=download_status,
            http_status=http_status,
            content_type=content_type,
            content_disposition=content_disposition,
            file_name_detected=file_name_detected,
            saved_path=saved_path,
            file_size_bytes=file_size_bytes,
            file_sha256=file_sha256,
            already_existed=already_existed,
            was_downloaded_now=was_downloaded_now,
            is_duplicate_uri=is_duplicate_uri,
            is_duplicate_file=is_duplicate_file,
            conflict_detected=conflict_detected,
            error_type=error_type,
            error_message=error_message,
            downloaded_at_utc=self._utc_now(),
        )

    def process_group(
        self,
        group: Dict[str, object],
        known_file_hash_to_path: Dict[str, str],
    ) -> DocumentDownloadRecord:
        row = group["first_row"]  # type: ignore[assignment]
        assert isinstance(row, dict)

        raw_uri = str(row.get("uri") or "")
        uri = self.normalize_uri(raw_uri)
        uri_sha256 = str(group["uri_sha256"])
        competencia = row.get("competencia", "")
        duplicate_uri_count = int(group["duplicate_uri_count"])
        is_duplicate_uri = bool(group["is_duplicate_uri"])

        existing = self.existing_file_for_uri(competencia, uri_sha256)

        if existing and self.skip_existing and not self.overwrite:
            content = existing.read_bytes()
            file_sha = self.sha256_bytes(content)

            return self._record(
                row=row,
                uri=uri,
                uri_sha256=uri_sha256,
                duplicate_uri_count=duplicate_uri_count,
                download_attempted=False,
                download_status="SKIPPED_EXISTING_URI",
                http_status=None,
                content_type=None,
                content_disposition=None,
                file_name_detected=existing.name,
                saved_path=str(existing),
                file_size_bytes=len(content),
                file_sha256=file_sha,
                already_existed=True,
                was_downloaded_now=False,
                is_duplicate_uri=is_duplicate_uri,
                is_duplicate_file=file_sha in known_file_hash_to_path,
                conflict_detected=False,
                error_type=None,
                error_message=None,
            )

        if not self.is_supported_download_uri(uri):
            return self._record(
                row=row,
                uri=uri,
                uri_sha256=uri_sha256,
                duplicate_uri_count=duplicate_uri_count,
                download_attempted=False,
                download_status="UNSUPPORTED_URI",
                http_status=None,
                content_type=None,
                content_disposition=None,
                file_name_detected=None,
                saved_path=None,
                file_size_bytes=0,
                file_sha256=None,
                already_existed=False,
                was_downloaded_now=False,
                is_duplicate_uri=is_duplicate_uri,
                is_duplicate_file=False,
                conflict_detected=False,
                error_type="UnsupportedUri",
                error_message="URI não é http(s) e não pôde ser baixada.",
            )

        if self.dry_run:
            return self._record(
                row=row,
                uri=uri,
                uri_sha256=uri_sha256,
                duplicate_uri_count=duplicate_uri_count,
                download_attempted=False,
                download_status="DRY_RUN",
                http_status=None,
                content_type=None,
                content_disposition=None,
                file_name_detected=None,
                saved_path=None,
                file_size_bytes=0,
                file_sha256=None,
                already_existed=False,
                was_downloaded_now=False,
                is_duplicate_uri=is_duplicate_uri,
                is_duplicate_file=False,
                conflict_detected=False,
                error_type=None,
                error_message=None,
            )

        try:
            response = self.session.get(uri, timeout=self.timeout_seconds)
            http_status = int(getattr(response, "status_code", 0) or 0)
            headers = getattr(response, "headers", {}) or {}
            content_type = headers.get("content-type") or headers.get("Content-Type")
            content_disposition = headers.get("content-disposition") or headers.get("Content-Disposition")
            file_name_detected = (
                self.filename_from_content_disposition(content_disposition)
                or self.filename_from_uri(uri)
            )

            if http_status < 200 or http_status >= 300:
                return self._record(
                    row=row,
                    uri=uri,
                    uri_sha256=uri_sha256,
                    duplicate_uri_count=duplicate_uri_count,
                    download_attempted=True,
                    download_status="HTTP_ERROR",
                    http_status=http_status,
                    content_type=content_type,
                    content_disposition=content_disposition,
                    file_name_detected=file_name_detected,
                    saved_path=None,
                    file_size_bytes=0,
                    file_sha256=None,
                    already_existed=False,
                    was_downloaded_now=False,
                    is_duplicate_uri=is_duplicate_uri,
                    is_duplicate_file=False,
                    conflict_detected=False,
                    error_type="HttpError",
                    error_message=f"HTTP status {http_status}",
                )

            content = bytes(getattr(response, "content", b"") or b"")

            if len(content) == 0:
                return self._record(
                    row=row,
                    uri=uri,
                    uri_sha256=uri_sha256,
                    duplicate_uri_count=duplicate_uri_count,
                    download_attempted=True,
                    download_status="EMPTY_CONTENT",
                    http_status=http_status,
                    content_type=content_type,
                    content_disposition=content_disposition,
                    file_name_detected=file_name_detected,
                    saved_path=None,
                    file_size_bytes=0,
                    file_sha256=None,
                    already_existed=False,
                    was_downloaded_now=False,
                    is_duplicate_uri=is_duplicate_uri,
                    is_duplicate_file=False,
                    conflict_detected=False,
                    error_type="EmptyContent",
                    error_message="Resposta HTTP sem conteúdo.",
                )

            if not self.is_expected_content_type(content_type):
                return self._record(
                    row=row,
                    uri=uri,
                    uri_sha256=uri_sha256,
                    duplicate_uri_count=duplicate_uri_count,
                    download_attempted=True,
                    download_status="UNEXPECTED_CONTENT_TYPE",
                    http_status=http_status,
                    content_type=content_type,
                    content_disposition=content_disposition,
                    file_name_detected=file_name_detected,
                    saved_path=None,
                    file_size_bytes=len(content),
                    file_sha256=self.sha256_bytes(content),
                    already_existed=False,
                    was_downloaded_now=False,
                    is_duplicate_uri=is_duplicate_uri,
                    is_duplicate_file=False,
                    conflict_detected=False,
                    error_type="UnexpectedContentType",
                    error_message=f"Content-Type inesperado: {content_type}",
                )

            file_sha = self.sha256_bytes(content)

            if file_sha in known_file_hash_to_path:
                return self._record(
                    row=row,
                    uri=uri,
                    uri_sha256=uri_sha256,
                    duplicate_uri_count=duplicate_uri_count,
                    download_attempted=True,
                    download_status="DUPLICATE_FILE_SHA",
                    http_status=http_status,
                    content_type=content_type,
                    content_disposition=content_disposition,
                    file_name_detected=file_name_detected,
                    saved_path=known_file_hash_to_path[file_sha],
                    file_size_bytes=len(content),
                    file_sha256=file_sha,
                    already_existed=True,
                    was_downloaded_now=True,
                    is_duplicate_uri=is_duplicate_uri,
                    is_duplicate_file=True,
                    conflict_detected=False,
                    error_type=None,
                    error_message=None,
                )

            target_path = self.target_path_for_download(
                competencia=competencia,
                uri_sha256=uri_sha256,
                content_type=content_type,
            )

            if target_path.exists() and not self.overwrite:
                current = target_path.read_bytes()
                current_sha = self.sha256_bytes(current)
                conflict = current_sha != file_sha

                return self._record(
                    row=row,
                    uri=uri,
                    uri_sha256=uri_sha256,
                    duplicate_uri_count=duplicate_uri_count,
                    download_attempted=True,
                    download_status="TARGET_EXISTS_CONFLICT" if conflict else "TARGET_EXISTS_SAME_HASH",
                    http_status=http_status,
                    content_type=content_type,
                    content_disposition=content_disposition,
                    file_name_detected=file_name_detected,
                    saved_path=str(target_path),
                    file_size_bytes=len(current),
                    file_sha256=current_sha,
                    already_existed=True,
                    was_downloaded_now=False,
                    is_duplicate_uri=is_duplicate_uri,
                    is_duplicate_file=current_sha in known_file_hash_to_path,
                    conflict_detected=conflict,
                    error_type="OverwriteBlocked" if conflict else None,
                    error_message="Arquivo alvo já existe e overwrite=False." if conflict else None,
                )

            target_path.parent.mkdir(parents=True, exist_ok=True)
            target_path.write_bytes(content)
            known_file_hash_to_path[file_sha] = str(target_path)

            return self._record(
                row=row,
                uri=uri,
                uri_sha256=uri_sha256,
                duplicate_uri_count=duplicate_uri_count,
                download_attempted=True,
                download_status="SUCCESS",
                http_status=http_status,
                content_type=content_type,
                content_disposition=content_disposition,
                file_name_detected=file_name_detected,
                saved_path=str(target_path),
                file_size_bytes=len(content),
                file_sha256=file_sha,
                already_existed=False,
                was_downloaded_now=True,
                is_duplicate_uri=is_duplicate_uri,
                is_duplicate_file=False,
                conflict_detected=False,
                error_type=None,
                error_message=None,
            )

        except Exception as exc:
            return self._record(
                row=row,
                uri=uri,
                uri_sha256=uri_sha256,
                duplicate_uri_count=duplicate_uri_count,
                download_attempted=True,
                download_status="REQUEST_EXCEPTION",
                http_status=None,
                content_type=None,
                content_disposition=None,
                file_name_detected=None,
                saved_path=None,
                file_size_bytes=0,
                file_sha256=None,
                already_existed=False,
                was_downloaded_now=False,
                is_duplicate_uri=is_duplicate_uri,
                is_duplicate_file=False,
                conflict_detected=False,
                error_type=exc.__class__.__name__,
                error_message=str(exc),
            )

    def download_documents(self) -> List[DocumentDownloadRecord]:
        timer = ExecutionTimer()

        rows = self.filter_download_links()
        groups = self.group_by_uri_sha256(rows)
        known_hashes = self.known_file_hashes()

        self.logger.started(
            layer="raw",
            module=self.MODULE,
            class_name=self.CLASS_NAME,
            method_name="download_documents",
            operation="download_direct_documents",
            message="Iniciando download idempotente dos documentos diretos.",
            records_in=len(rows),
        )

        records: List[DocumentDownloadRecord] = []

        for group in groups:
            records.append(self.process_group(group, known_hashes))

        self.records = records

        self.logger.success(
            layer="raw",
            module=self.MODULE,
            class_name=self.CLASS_NAME,
            method_name="download_documents",
            operation="download_direct_documents",
            message="Download idempotente dos documentos diretos concluído.",
            records_in=len(rows),
            records_out=len(records),
            duration_ms=timer.elapsed_ms(),
        )

        return records

    def build_records(self) -> List[Dict[str, object]]:
        if not self.records:
            self.download_documents()

        return [asdict(record) for record in self.records]

    def write_csv(self, rows: List[Dict[str, object]], path: Path) -> Path:
        path.parent.mkdir(parents=True, exist_ok=True)

        fieldnames = list(DocumentDownloadRecord.__dataclass_fields__.keys())

        with path.open("w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=fieldnames)
            writer.writeheader()
            writer.writerows(rows)

        return path

    def write_parquet(self, rows: List[Dict[str, object]], path: Path) -> Optional[Path]:
        try:
            import pyarrow as pa
            import pyarrow.parquet as pq
        except ImportError as exc:
            if self.require_parquet:
                raise RuntimeError(
                    "pyarrow não está instalado. Instale com: python -m pip install pyarrow"
                ) from exc
            return None

        path.parent.mkdir(parents=True, exist_ok=True)
        table = pa.Table.from_pylist(rows)
        pq.write_table(table, path)
        return path

    def write_output(self, rows: List[Dict[str, object]]) -> Dict[str, Optional[str]]:
        timer = ExecutionTimer()

        run_dir = self.output_dir / f"run_id={self.logger.run_id}"
        csv_path = run_dir / "document_downloads.csv"
        parquet_path = run_dir / "document_downloads.parquet"
        latest_csv_path = self.output_dir / "document_downloads_latest.csv"

        self.write_csv(rows, csv_path)
        self.write_csv(rows, latest_csv_path)
        parquet_written = self.write_parquet(rows, parquet_path)

        self.logger.success(
            layer="raw",
            module=self.MODULE,
            class_name=self.CLASS_NAME,
            method_name="write_output",
            operation="write_document_downloads",
            message="Registro RAW de downloads documentais gravado.",
            records_out=len(rows),
            duration_ms=timer.elapsed_ms(),
            source_file=str(csv_path),
        )

        return {
            "csv_path": str(csv_path),
            "latest_csv_path": str(latest_csv_path),
            "parquet_path": str(parquet_written) if parquet_written else None,
        }

    def run(self) -> Dict[str, Optional[str]]:
        timer = ExecutionTimer()

        self.logger.started(
            layer="raw",
            module=self.MODULE,
            class_name=self.CLASS_NAME,
            method_name="run",
            operation="direct_document_downloader_pipeline",
            message="Pipeline de download direto de documentos iniciado.",
        )

        try:
            rows = self.build_records()
            outputs = self.write_output(rows)

            self.logger.success(
                layer="raw",
                module=self.MODULE,
                class_name=self.CLASS_NAME,
                method_name="run",
                operation="direct_document_downloader_pipeline",
                message="Pipeline de download direto de documentos finalizado.",
                records_out=len(rows),
                duration_ms=timer.elapsed_ms(),
            )

            self.logger.close(require_parquet=self.require_parquet)
            return outputs

        except Exception as exc:
            self.logger.failed(
                layer="raw",
                module=self.MODULE,
                class_name=self.CLASS_NAME,
                method_name="run",
                operation="direct_document_downloader_pipeline",
                message="Pipeline de download direto de documentos falhou.",
                error_type=exc.__class__.__name__,
                error_message=str(exc),
                duration_ms=timer.elapsed_ms(),
            )
            self.logger.close(require_parquet=False)
            raise
